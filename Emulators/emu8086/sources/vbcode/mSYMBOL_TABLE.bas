...
On Error GoTo error_save_stf
    ...
    sFilename = sFilename & ".symbol" ' adding ".symbol" extention:
...
On Error GoTo error_load_stf
    ...
    ' #327xo-av-protect-4#
    If FileExists(sFilename & ".symbol") Then
        sFilename = sFilename & ".symbol" ' adding ".symbol" extention. OK!
    Else
        sFilename = sFilename & "_.symbol" ' try this one.... (the executable file could be renamed).
    End If
