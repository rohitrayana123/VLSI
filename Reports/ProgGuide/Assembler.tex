...
Branches may occur within the ISR, if within an accepted distance\todo{HSL:what's an ``accepted'' distance?}, but are not allowed into this subroutine with the exception of a RET call. \todo{HSL: this last bit makes no sense}\\
...
\todo{HSL: I don't think the above is needed. If it is automatically taken care of, the programmer does not need to know surely?}

\subsection{Assembler Directives}
%There is one supported assembler directive  for assigning meaningful names to each of the general purpose registers. 
\todo{HSL: are branch labels not assembler directives?} 
...
\todo[inline]{HSL: I've changed this to use the standard executable method that Unix users will be comfortable with}
