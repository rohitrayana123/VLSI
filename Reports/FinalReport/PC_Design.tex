...
\noindent The program counter sources are:
\begin{enumerate}
...
\item System bus - for returning the program counter from the stack after an interrupt
\item ISR Location - a constant defined for the location of the interrupt service routine in memory.
\end{enumerate}

\noindent The link register has two inputs:
\begin{enumerate}
...
\todo[inline, color=green]{MW:Ive reworded this to better wexplain use of muxes over tristates}
\todo[inline, color=green]{MW:Some of this is surely implied, could you not say more? @seblovett}
