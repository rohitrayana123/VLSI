...
Two tristate buffers are used per register onto a bus for the two read data outputs. 
...
The circuit diagram is seen in Figure~\ref{fig:reg:decoder}.
This is for one of the decoders needed. 
The two read decoders are identical.
However, the write decoder must also use a write enable function.
This is done by using the same logic functions seen in Table~\ref{tab:reg:decoder}, but the resulting signals are then \texttt{AND}ed with the register write enable signal. 
...
%Layout in silicon
The layout in silicon consisted of an array of the slices. 
...
The decoder was designed to fit on top of the register slice. 
